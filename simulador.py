...
class Torneio:
    ...
    def monitor(self, fim=False):
        ...
        """
        window_size = 2000
        if self.rodada % 100 == 0 or fim:
            com_series = [self.historico[nome]["comida"][-window_size:] for nome in
                          jogadores]  # if nome not in self.cemiterio]
    ...
    def atualiza_reputacao(self):
        for nome in self.jogadores.keys():
            self.historico[nome]["reputacao"].append(self.historico[nome]["cacou"] / (
                float(self.historico[nome]["cacou"] + self.historico[nome]["descansou"])))
    ...
    def vai(self):
        ...
        while True:
            ...
            if self.checa_fim():
                ...
                break
            elif self.rodada > self.max_rodadas:
    ...
    def anuncia_vencedor(self):
        ...
        ranking1 = [(nome, data["comida"][-1]) for nome, data in self.historico.items() if
                    (data["comida"][-1] > 0 and nome not in self.cemiterio and nome not in self.bugados)]
