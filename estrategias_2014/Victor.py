...
class Jogador:
    def __init__(self):
        ...
        self.comida = 0
        self.reputacao = 0
        self.reputacao_minima = 0.4
        self.nivel_risco_1 = 0.3
        self.nivel_risco_2 = 0.7
    ...
    def escolha_de_cacada(self, rodada, comida_atual, reputacao_atual, m, reputacoes_dos_jogadores):
        ...
        r = ['d','c']
        if rodada == 1:
            escolhas = [r[(int) (random.random() + 0.5)] for x in reputacoes_dos_jogadores]
            return escolhas
        else:
            if reputacao <= reputacao_minima:
                for i in range(len(reputacoes_dos_jogadores)):
                    if reputacoes_dos_jogadores[i] <= nivel_risco_1:
                        escolhas = ['d' for x in reputacoes_dos_jogadores]
                        return escolhas
                    else:
                        if reputacoes_dos_jogadores[i] <= nivel_risco_2:
                            escolhas = [r[(int) (random.random() + reputacoes_dos_jogadores[i])] for x in reputacoes_dos_jogadores]
                            return escolhas
                        else:
