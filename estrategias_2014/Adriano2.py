...
class MeuJogador(Jogador):
    ...
    def escolha_de_cacada(self, rodada, comida_atual, reputacao_atual, m, reputacoes_dos_jogadores):
        ...
        if reputacao_atual <= 0.35:
            if media_reputacoes < 0.20:
                if m <= (m_max/4):
                    Mini, Maxi = 0.10, 0.70
                else:
                    Mini, Maxi = 2*media_reputacoes, 0.70
                    
            if media_reputacoes > 0.55:
                if comida_atual < comida_atras:
                    Mini, Maxi = media_reputacoes, 1.00
                else:
                    Mini, Maxi = media_reputacoes - 0.20, 1.00
                
            else:
                Mini, Maxi = 0.30, 0.80
        
        elif reputacao_atual >= 0.75:
